def convert_num_to_english(num: int) -> str:
    ...
    if num < 1000:
        return f"{nums_to_words_dict.get(num // 100,'')} hundred {convert_num_to_english(num % 100) if num % 100 > 0 else ''}".strip()

    if num < mil:
        return f"{convert_num_to_english(num // 1000)} thousand {convert_num_to_english(num % 1000) if num % 1000 > 0 else ''}".strip()
    
    if num < bil:
        return f"{convert_num_to_english(num // mil)} million {convert_num_to_english(num % mil) if num % mil > 0 else ''}".strip()